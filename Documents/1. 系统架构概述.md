...
2. 对象存储现在保留并实现了 MinIO：默认 provider 为 `MINIO`，实现类为 `MinioObjectStorageService`。
3. 本地文件系统仍作为兼容实现保留：provider 为 `LOCAL_FS`，实现类为 `LocalObjectStorageService`，主要用于 e2e、开发共享卷或不启动 MinIO 的临时环境。
...
| `caries-image` | 附件上传、访问 URL、内容下载、病例影像、影像质检、对象存储 | `AttachmentAppService`、`MinioObjectStorageService`、`LocalObjectStorageService` |
...
| MySQL | 已实现 | Flyway V001-V014 管理结构和种子 |
| RabbitMQ | 已接入 | `RabbitAnalysisTaskEventPublisher` 发布 AI 分析任务消息 |
| MinIO | 已实现并默认启用 | `provider-code=MINIO`，`MinioObjectStorageService` 负责对象读写 |
...
3. `AttachmentAppService.createAccessUrl` 面向用户端生成短时 URL。
4. `AttachmentAppService.createInternalAccessUrl` 面向 Java/Python 内部联调生成短时 URL。
5. `AttachmentAppService.resolveLocalStoragePath` 只服务 LOCAL_FS 场景，MinIO 场景不依赖共享卷。
6. `/api/v1/files/{attachmentId}/content` 根据签名参数读取对象内容。
...
5. 前端或外部消费者下载时调用 `/api/v1/files/{attachmentId}/content?expireAt=...&signature=...`。
...
1. 不再写“来源使用 MinIO，但现状只有 LocalObjectStorageService”。现状是 MinIO 已保留并实现，LocalFS 为兼容 provider。
