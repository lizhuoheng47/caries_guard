...
5. `ModelAdmin` 的完整模型治理平台没有独立模块，但数据库基线已补入最小治理落点：`ana_model_version_registry`、任务 `model_version`、看板模型运行统计、回调 `traceId` 与 `inferenceMillis`。
...
8. 系统权限种子已经由数据库基线补齐基础菜单、业务角色和数据权限规则，不再是“只能稳定演示管理员”的状态。
...
| MySQL | 已实现 | Flyway 单基线 `V001__baseline_schema.sql` 管理结构和种子 |
...
3. 数据库基线初始化 `ORG_ADMIN`、`DOCTOR`、`SCREENER` 三类业务角色。
4. 数据库基线初始化患者、就诊、病例、影像、分析任务、报告、随访、看板、AI 运行看板菜单。
...
4. 不再写“菜单种子为空、只能管理员演示”。现状是数据库基线已补菜单、角色和权限规则种子。
