...
class GraphRetriever:
        ...
        with self.driver.session(database=self.settings.neo4j_database) as session:
            for entity in linked_entities:
                ...
                for template in self.template_registry.matching_templates(entity_type):
                    if template.required_entity_types and entity_type not in template.required_entity_types:
                        continue
    ...
    def _records_to_evidence(
        ...
        for idx, record in enumerate(records[:top_k], start=1):
            ...
            score = (1.0 / idx) + float(chunk.get("confidenceScore") or 0.0) * self.settings.rag_graph_confidence_weight + authority * 0.05
            evidence.append(
                {
