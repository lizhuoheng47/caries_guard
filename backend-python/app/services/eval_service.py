...
class EvalService:
    ...
    def run_dataset(self, dataset_id: int, org_id: int | None, created_by: int | None) -> dict:
        ...
        for question in questions:
            ...
            actual_citations = {item.get("documentCode") for item in answer.get("citations", [])}
            expected_citations = {item.get("expected_doc_no") for item in question.get("expected_citations", []) if item.get("expected_doc_no")}
            citation_hit = "1" if not expected_citations or bool(actual_citations & expected_citations) else "0"
            ...
            self.eval_repository.save_result(
                ...
                latency_ms=answer.get("latencyMs"),
                metric_json={
