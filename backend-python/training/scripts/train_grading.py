...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train caries grading model")
    parser.add_argument(
        "--dataset-manifest",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(PROJECT_ROOT / "training" / "outputs" / "runs" / "grading_v1"),
...
def main() -> None:
    ...
    logger = configure_logger("training.grading.train", output_dirs["logs"] / "train.log")
    ...
    logger.info(
        "starting grading training train_samples=%s val_samples=%s input_size=%s device=%s output_dir=%s",
        len(train_records),
        len(val_records),
        args.input_size,
        args.device,
    ...
    write_json(
        ...
        build_confusion_matrix_payload(result["bestConfusionMatrix"], grade_labels),
    )
    logger.info(
        "grading training finished best_epoch=%s best_checkpoint=%s",
        result["bestEpoch"],
