...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate caries segmentation model")
    parser.add_argument(
        "--dataset-manifest",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(PROJECT_ROOT / "training" / "outputs" / "runs" / "seg_v1_eval"),
    ...
    parser.add_argument(
        "--checkpoint",
        default=str(PROJECT_ROOT / "training" / "outputs" / "runs" / "seg_v1" / "checkpoints" / "best.pt"),
...
def main() -> None:
    ...
    logger = configure_logger("training.segmentation.eval", output_dirs["logs"] / "evaluate.log")
    ...
    logger.info(
        "starting segmentation evaluation samples=%s checkpoint=%s input_size=%s device=%s",
        len(records),
        args.checkpoint,
        args.input_size,
        args.device,
    ...
    model, device, checkpoint_payload = runtime.load_segmentation_checkpoint(args.checkpoint, args.device)
    data_loader = runtime.make_segmentation_loader(
        records=records,
    ...
    metrics, confusion = runtime.evaluate_segmentation_model(
        model=model,
    ...
    write_json(
        ...
        build_confusion_matrix_payload(confusion.tolist(), segmentation_labels),
    )
    write_json(output_dirs["metrics"] / "evaluation_args.json", vars(args))
    logger.info("segmentation evaluation finished checkpoint=%s", args.checkpoint)
