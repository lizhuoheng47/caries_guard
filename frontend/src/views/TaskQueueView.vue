  ...
  <div class="flex flex-col h-full p-4 lg:p-6 pb-0 overflow-hidden page-bg">
    ...
    <div class="flex items-center justify-between mb-4 shrink-0">
      <div class="flex flex-col">
        <span class="font-mono text-[11px] text-[var(--td)] tracking-[0.1em] uppercase mb-1">AI 核心 / <span class="text-[var(--cyan)]">流水线</span></span>
        <h2 class="text-[22px] font-medium text-[var(--tp)] m-0">分析队列</h2>
      </div>
    </div>
    ...
    <div class="flex items-center justify-between mb-4 shrink-0">
      ...
      <div class="relative w-[320px]">
        <input 
          type="text" 
          placeholder="搜索任务 ID 或患者..." 
          class="w-full bg-[rgba(3,8,18,0.7)] border border-[var(--ln)] rounded-[3px] h-[36px] pl-9 pr-12 text-[13px] text-[var(--tp)] focus:outline-none focus:border-[var(--cyan)] focus:shadow-[0_0_8px_rgba(0,229,255,0.2)] transition-all font-mono placeholder:text-[var(--td)]"
        />
        ...
        <div class="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 font-mono text-[10px] text-[var(--td)] border border-[var(--ln)] px-1 rounded-xs bg-[var(--void)]">
      ...
      <div class="flex gap-2 p-1 border border-[var(--ln)] rounded-[4px] bg-[rgba(3,8,18,0.5)]">
        <button class="px-3 py-1 font-mono text-[11px] uppercase rounded-xs transition-colors flex items-center gap-1.5 text-[var(--td)] hover:text-[var(--tp)]">
          全部 <span class="text-[9px] opacity-60">2847</span>
        </button>
        <button class="px-3 py-1 font-mono text-[11px] uppercase rounded-xs transition-colors flex items-center gap-1.5 bg-[var(--cyan)]/20 text-[var(--cyan)] shadow-[0_0_8px_rgba(0,229,255,0.2)]">
          完成 <span class="text-[9px] opacity-80">2610</span>
        </button>
        <button class="px-3 py-1 font-mono text-[11px] uppercase rounded-xs transition-colors flex items-center gap-1.5 text-[var(--td)] hover:text-[var(--tp)]">
          运行中 <span class="text-[9px] opacity-60">12</span>
        </button>
        <button class="px-3 py-1 font-mono text-[11px] uppercase rounded-xs transition-colors flex items-center gap-1.5 text-[var(--td)] hover:text-[var(--tp)]">
          复核 <span class="text-[9px] opacity-60">205</span>
        </button>
        <button class="px-3 py-1 font-mono text-[11px] uppercase rounded-xs transition-colors flex items-center gap-1.5 text-[var(--td)] hover:text-[var(--tp)]">
          失败 <span class="text-[9px] opacity-60">20</span>
        </button>
      </div>
    </div>
    ...
    <div class="flex-1 min-h-0 glass-panel rounded-md flex flex-col overflow-hidden">
      <table class="w-full text-left border-collapse flex-1 relative">
        <thead class="bg-[rgba(3,8,18,0.7)] sticky top-0 z-10 shadow-sm border-b border-[var(--ln)]">
          <tr>
            <th class="font-mono text-[11px] text-[var(--ts)] uppercase tracking-[0.12em] py-3 px-4 font-normal">任务 ID</th>
            <th class="font-mono text-[11px] text-[var(--ts)] uppercase tracking-[0.12em] py-3 px-4 font-normal">患者</th>
            <th class="font-mono text-[11px] text-[var(--ts)] uppercase tracking-[0.12em] py-3 px-4 font-normal">分级</th>
            <th class="font-mono text-[11px] text-[var(--ts)] uppercase tracking-[0.12em] py-3 px-4 font-normal">不确定度</th>
            <th class="font-mono text-[11px] text-[var(--ts)] uppercase tracking-[0.12em] py-3 px-4 font-normal">状态</th>
            <th class="font-mono text-[11px] text-[var(--ts)] uppercase tracking-[0.12em] py-3 px-4 font-normal">创建时间</th>
            <th class="font-mono text-[11px] text-[var(--ts)] uppercase tracking-[0.12em] py-3 px-4 font-normal">耗时</th>
            <th class="font-mono text-[11px] text-[var(--ts)] uppercase tracking-[0.12em] py-3 px-4 font-normal text-right">操作</th>
          </tr>
        </thead>
        <tbody class="overflow-y-auto block w-full absolute top-[36px] bottom-0" style="display: table-row-group;">
          <tr v-if="loading">
            <td colspan="8" class="text-center py-8 text-[var(--td)]">神经流水线数据加载中...</td>
          </tr>
          <tr v-else-if="tasks.length === 0">
            <td colspan="8" class="text-center py-8 text-[var(--td)]">未找到任务。</td>
          </tr>
          ...
          <tr v-for="(task, i) in tasks" :key="task.taskId" class="border-b border-[var(--ln)] hover:bg-[rgba(0,229,255,0.02)] transition-colors relative">
            ...
            <td class="py-3 px-4">
              <GradeBadge :grade="task.gradingLabel || '—'" />
            </td>
            <td class="py-3 px-4 w-[140px]">
              <div class="flex items-center gap-2">
                <span class="font-mono text-[10px]" :class="(task.uncertaintyScore ?? 0) > 0.35 ? 'text-[var(--amber)]' : 'text-[var(--ts)]'">{{ (task.uncertaintyScore ?? 0).toFixed(2) }}</span>
                <div class="flex-1 h-[4px] bg-[var(--void)] border border-[var(--ln)] rounded-full overflow-hidden">
                  <div class="h-full" :class="(task.uncertaintyScore ?? 0) > 0.35 ? 'bg-[var(--amber)] shadow-[0_0_4px_var(--amber)]' : 'bg-[var(--cyan)] shadow-[0_0_4px_var(--cyan)]'" :style="{ width: `${(task.uncertaintyScore ?? 0) * 100}%` }"></div>
                </div>
              </div>
            </td>
            <td class="py-3 px-4">
    ...
    <div class="shrink-0 h-[40px] flex items-center justify-between px-2">
      <span class="font-mono text-[11px] text-[var(--td)] tracking-widest uppercase">显示 {{ (pageNo - 1) * 10 + 1 }}-{{ Math.min(pageNo * 10, total) }} 条，共 {{ total }} 条 · 按创建时间排序 ↓</span>
