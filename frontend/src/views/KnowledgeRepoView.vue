<template>
  <div class="flex flex-col h-full p-4 lg:p-6 pb-0 overflow-hidden page-bg">
      ...
      <div class="flex items-center gap-3">
        <div class="flex flex-col">
          <span class="font-mono text-[11px] text-[var(--td)] tracking-[0.1em] uppercase mb-1">
            Knowledge Core / <span class="text-[var(--violet)]">Repository</span>
          </span>
          <div class="flex items-center gap-2">
            ...
            <div class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-[2px] border bg-[var(--violet)]/10 border-[var(--violet)]/30">
              ...
              <span class="font-mono text-[10px] text-[var(--violet)] uppercase tracking-[0.1em]">RAG Engine</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="grid grid-cols-4 gap-3 mb-4 shrink-0">
      <KpiCard label="文档总数" :value="statsDocs" color="violet" />
      <KpiCard label="切片总数" :value="statsChunks" color="cyan" />
      <KpiCard label="最近索引" :value="statsLastIndexed" color="emerald" />
      <KpiCard label="知识版本" :value="statsVersion" color="amber" />
    </div>
    ...
    <div class="flex-1 glass-panel rounded-md overflow-hidden flex flex-col min-h-0">
      <div
        ...
        :class="isDragActive ? 'border-[var(--violet)] bg-[var(--violet)]/15' : 'border-[var(--violet)]/20 bg-[var(--violet)]/5 hover:bg-[var(--violet)]/10'"
      ...
      >
        ...
        <span class="text-[13px] text-[var(--tp)] mb-1">
          {{ store.uploading ? '正在上传文档...' : '拖拽文档到此或点击上传' }}
        </span>
        <span class="text-[11px] text-[var(--td)]">支持 PDF / DOC / DOCX / TXT / MD</span>
      </div>
      ...
      <div class="overflow-auto flex-1 relative">
        <table class="w-full text-left border-collapse min-w-[900px]">
          <thead class="sticky top-0 bg-[rgba(3,8,18,0.85)] backdrop-blur z-10 border-b border-[var(--ln)]">
            <tr>
              <th class="py-2.5 pl-4 px-3 text-[10px] font-mono text-[var(--ts)] uppercase tracking-[0.12em] font-normal">文档</th>
              ...
              <th class="py-2.5 px-3 text-[10px] font-mono text-[var(--ts)] uppercase tracking-[0.12em] font-normal text-right">切片</th>
              <th class="py-2.5 px-3 text-[10px] font-mono text-[var(--ts)] uppercase tracking-[0.12em] font-normal text-right">实体</th>
          ...
          <tbody>
            ...
            >
              ...
              <td class="py-3 px-3">
                <span class="font-mono text-[10px] px-1.5 py-0.5 rounded-[2px] border" :class="getTypeClass(doc.type)">
              ...
              <td class="py-3 px-3 text-right font-mono text-[11px] text-[var(--tp)] tabular-nums">{{ formatCount(doc.chunks) }}</td>
              <td class="py-3 px-3 text-right font-mono text-[11px] text-[var(--tp)] tabular-nums">{{ formatCount(doc.entities) }}</td>
              <td class="py-3 px-3 text-center font-mono text-[10px] text-[var(--ts)]">{{ doc.version || '--' }}</td>
              <td class="py-3 px-3 font-mono text-[10px] text-[var(--td)]">{{ formatDateTime(doc.updatedAt) }}</td>
              <td class="py-3 pr-4 pl-2 text-right">
                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button class="text-[10px] font-mono text-[var(--violet)] hover:underline transition-colors disabled:opacity-50 disabled:no-underline" :disabled="store.isDocBusy(doc.id)" @click="openDocumentDetail(doc)">
                    查看
                  </button>
                  <button class="text-[10px] font-mono text-[var(--td)] hover:text-[var(--tp)] transition-colors disabled:opacity-50" :disabled="store.isDocBusy(doc.id)" @click="handleReindexDoc(doc)">
                    重建
                  </button>
                  <button class="text-[10px] font-mono text-[var(--magenta)] hover:underline transition-colors disabled:opacity-50" :disabled="store.isDocBusy(doc.id)" @click="handleDeleteDoc(doc)">
                    删除
                  </button>
                </div>
              </td>
            ...
            <tr v-if="!store.loading && store.documents.items.length === 0">
              <td colspan="8" class="py-16 text-center">
                <div class="flex flex-col items-center gap-2">
                  ...
                  <span class="text-[13px] text-[var(--td)]">当前知识库暂无文档。</span>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <div class="h-12 border-t border-[var(--ln)] bg-[rgba(3,8,18,0.5)] flex items-center justify-between px-4 shrink-0">
        <div class="font-mono text-[11px] text-[var(--td)] uppercase tracking-[0.1em]">
          显示 {{ displayStart }}-{{ displayEnd }} / {{ store.documents.total }}
        </div>

        <div class="flex items-center gap-1">
          <button class="w-6 h-6 flex items-center justify-center rounded-xs border border-[var(--ln)] text-[var(--td)] transition-colors disabled:opacity-40 disabled:cursor-not-allowed hover:text-[var(--tp)]" :disabled="!canPrev" @click="goPrev">
            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
            </svg>
          </button>
          <button class="w-8 h-6 flex items-center justify-center rounded-xs border border-[var(--violet)] bg-[var(--violet)]/10 text-[var(--violet)] font-mono text-[10px] shadow-[0_0_8px_rgba(139,92,246,0.2)]">
            {{ store.documents.page }}
          </button>
          <button class="w-8 h-6 flex items-center justify-center rounded-xs border border-[var(--ln)] text-[var(--td)] font-mono text-[10px]">
            {{ totalPages }}
          </button>
          <button class="w-6 h-6 flex items-center justify-center rounded-xs border border-[var(--ln)] text-[var(--td)] transition-colors disabled:opacity-40 disabled:cursor-not-allowed hover:text-[var(--tp)]" :disabled="!canNext" @click="goNext">
            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <template v-else-if="detailDoc">
              <div class="grid grid-cols-2 gap-3 mb-4">
                <div class="p-3 border border-[var(--ln)] rounded-[4px] bg-[rgba(3,8,18,0.55)]">
                  ...
                  <div class="text-[12px] text-[var(--tp)] leading-6">
                    ...
                    <div>文档类型: {{ typeMap[detailDoc.type] || detailDoc.type }}</div>
                    <div>当前版本: {{ detailDoc.version || '--' }}</div>
                ...
                <div class="p-3 border border-[var(--ln)] rounded-[4px] bg-[rgba(3,8,18,0.55)]">
                  ...
                  <div class="text-[12px] text-[var(--tp)] leading-6">
                    ...
                    <div>切片数量: {{ formatCount(detailDoc.chunks) }}</div>
                    <div>实体数量: {{ formatCount(detailDoc.entities) }}</div>
                    <div>关系数量: {{ formatCount(detailDoc.relations) }}</div>
                  </div>
                </div>
              </div>
              ...
              <div class="mb-4 p-3 border border-[var(--ln)] rounded-[4px] bg-[rgba(3,8,18,0.55)]">
                ...
                <textarea
                  ...
                  placeholder="可选：填写审核或发布备注"
                ></textarea>
              </div>

              <div class="flex flex-wrap items-center gap-2 mb-4">
                <NeuralButton variant="ghost" :disabled="detailActionLoading" @click="handleDetailAction('submitReview')">提交审核</NeuralButton>
                <NeuralButton variant="success" :disabled="detailActionLoading" @click="handleDetailAction('approve')">审核通过</NeuralButton>
                <NeuralButton variant="danger" :disabled="detailActionLoading" @click="handleDetailAction('reject')">驳回</NeuralButton>
                <NeuralButton variant="primary" :disabled="detailActionLoading" @click="handleDetailAction('publish')">发布</NeuralButton>
                <NeuralButton variant="ghost" :disabled="detailActionLoading" @click="handleDetailAction('rollback')">回滚</NeuralButton>
              </div>

              <div class="border border-[var(--ln)] rounded-[4px] overflow-hidden">
                <div class="h-9 px-3 flex items-center font-mono text-[10px] text-[var(--td)] uppercase border-b border-[var(--ln)]">版本记录</div>
                <div class="max-h-[220px] overflow-auto">
                  <table class="w-full border-collapse">
                    ...
                    <tbody>
                      <tr v-for="version in detailDoc.versions" :key="version.versionNo" class="border-b border-[var(--ln)]/50">
                        ...
                        <td class="py-2 px-3 font-mono text-[10px] text-[var(--td)]">{{ formatDateTime(version.updatedAt || version.createdAt) }}</td>
                      </tr>
...
const uploadFile = async (file?: File | null) => {
  ...
  try {
    ...
    notificationStore.success('上传成功', `${file.name} 已入库，后台将继续构建索引。`)
    await reload(1)
  } catch (error) {
...
const handleRebuildAll = async () => {
  const ok = window.confirm('确认重建知识索引吗？该操作会触发后台重建。')
  if (!ok) return

  try {
    await store.rebuildIndex()
    notificationStore.success('已提交重建', '后台正在重建索引。')
    await reload()
  } catch (error) {
...
const handleReindexDoc = async (doc: KnowledgeDocument) => {
  const ok = window.confirm(`确认对文档 ${doc.no} 重新触发索引吗？`)
  if (!ok) return

  try {
    await store.reindexDocument(doc)
    notificationStore.success('已重新索引', `${doc.no} 已重新进入索引流程。`)
    await reload()
  } catch (error) {
    notificationStore.error('重建失败', normalizedError(error))
  }
...
const handleDeleteDoc = async (doc: KnowledgeDocument) => {
  ...
  try {
    ...
    const targetPage = store.documents.items.length === 1 && store.documents.page > 1 ? store.documents.page - 1 : store.documents.page
...
const handleDetailAction = async (action: DetailAction) => {
  ...
  try {
    ...
    if (action === 'submitReview') {
      ...
      notificationStore.success('已提交审核', '文档已进入审核流。')
    } else if (action === 'approve') {
      await store.approve(detailDoc.value.id, payload)
      notificationStore.success('审核通过', '文档审核状态已更新。')
    } else if (action === 'reject') {
      await store.reject(detailDoc.value.id, payload)
      notificationStore.success('已驳回', '文档已被驳回。')
      ...
      notificationStore.success('回滚成功', '文档已回滚到上一个版本。')
    }
...
const getDotColor = (publishStatus: string, reviewStatus: string) => {
  if (publishStatus === 'PUBLISHED' || publishStatus === 'INDEXED') return 'bg-[var(--emerald)] shadow-[0_0_4px_var(--emerald)]'
  if (reviewStatus === 'REVIEW_PENDING' || reviewStatus === 'PARSED') return 'bg-[var(--cyan)] shadow-[0_0_4px_var(--cyan)] animate-pulse-opacity'
  if (reviewStatus === 'REJECTED') return 'bg-[var(--magenta)] shadow-[0_0_4px_var(--magenta)]'
