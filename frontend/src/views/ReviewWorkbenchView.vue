<template>
  <div class="flex h-full overflow-hidden page-bg">
    ...
    <div class="w-[260px] shrink-0 border-r border-[var(--ln)] flex flex-col" style="background: rgba(3,8,18,0.4)">
      <div class="p-4 border-b border-[var(--ln)]">
        <div class="flex items-center justify-between mb-2">
          ...
          <div class="px-1.5 py-0.5 rounded-xs bg-[var(--amber)]/20 text-[var(--amber)] font-mono text-[10px] border border-[var(--amber)]">07</div>
        </div>
        <!-- Search/Filter -->
        <div class="relative">
          <input type="text" placeholder="筛选列表..." class="w-full h-[32px] bg-[rgba(3,8,18,0.7)] border border-[var(--ln)] rounded-[3px] px-2 text-[12px] text-[var(--tp)] font-mono focus:border-[var(--amber)] focus:outline-none" />
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-2 flex flex-col gap-2">
        ...
        <div v-if="workbenchData" class="p-3 border rounded-[4px] border-[var(--amber)] bg-[var(--amber)]/5 shadow-[0_0_12px_rgba(255,181,71,0.15)] relative cursor-pointer">
          ...
          <div class="flex justify-between items-start mb-2 pl-1">
            <span class="font-mono text-[11px] text-[var(--amber)]">{{ workbenchData.task.taskNo || 'T-' + workbenchData.task.taskId }}</span>
            <span class="font-mono text-[9px] text-[var(--td)]">{{ new Date(workbenchData.task.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) }}</span>
          </div>
          <div class="flex items-center gap-2 pl-1">
            <GradeBadge :grade="workbenchData.aiResult.gradingLabel" />
            <div class="font-mono text-[10px] text-[var(--ts)] flex-1">{{ workbenchData.caseInfo.caseNo }}</div>
            <span class="font-mono text-[10px] text-[var(--amber)]">UC {{ workbenchData.aiResult.uncertaintyScore.toFixed(2) }}</span>
          </div>
        </div>
        ...
        <div v-for="i in 6" :key="i" class="p-3 border rounded-[4px] border-[var(--ln)] bg-[rgba(10,20,40,0.3)] hover:bg-[rgba(10,20,40,0.6)] cursor-pointer transition-colors">
          <div class="flex justify-between items-start mb-2">
            <span class="font-mono text-[11px] text-[var(--ts)]">T-2026041800{{ i+1 }}</span>
            <span class="font-mono text-[9px] text-[var(--td)]">13:{{ 50-i }}</span>
          </div>
          <div class="flex items-center gap-2">
            <GradeBadge :grade="i % 2 === 0 ? 'G2' : 'G4'" />
            <div class="font-mono text-[10px] text-[var(--td)] flex-1">P-100{{ 2+i }}</div>
            <span class="font-mono text-[10px] text-[var(--ts)]">UC 0.{{ 40+i*5 }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 flex flex-col min-w-0 p-4 lg:p-6">
      <div class="flex items-center justify-between mb-4 shrink-0">
        <div class="flex items-center gap-3">
          ...
          <span class="font-mono text-[12px] text-[var(--amber)] tracking-[0.1em]" v-if="workbenchData">{{ workbenchData.task.taskNo || 'T-' + workbenchData.task.taskId }}</span>
        </div>
      </div>
          ...
          <Panel title="影像扫描" color="cyan">
            <div class="relative w-full h-full rounded-[4px] bg-[#0A1428] overflow-hidden border-[0.5px] border-[var(--ln)]" style="background: radial-gradient(ellipse at center, #0A1428, #030812)">
              ...
              <div class="absolute pointer-events-none opacity-50" style="left: 45%; top: 30%; width: 10%; height: 35%;">
                ...
                <div class="absolute -top-4 left-1/2 -translate-x-1/2 bg-[var(--amber)] px-1 py-0.5 rounded-xs"><span class="font-mono text-[9px] text-[var(--void)]">AI: G3</span></div>
              </div>
              ...
              <div class="absolute pointer-events-none" style="left: 45%; top: 35%; width: 10%; height: 25%;">
                ...
                <div class="absolute -top-4 left-1/2 -translate-x-1/2 bg-[var(--emerald)] px-1 py-0.5 rounded-xs shadow-[0_0_8px_var(--emerald)]"><span class="font-mono text-[9px] text-[var(--void)] font-bold">DOC: G2</span></div>
              </div>
              
              <div class="absolute top-2 left-2 z-40 flex flex-col gap-2">
                <HudChip label="AI 原始预测 · 2 处病灶" color="amber" />
                <HudChip label="医师校对 · 1 处修正" color="emerald" />
              </div>
            </div>
          </Panel>
        </div>
            ...
            <div class="flex flex-col h-full gap-4">
              ...
              <div class="flex items-center justify-between p-3 border border-[var(--ln)] rounded-[4px] bg-[rgba(10,20,40,0.5)]">
                <div class="flex flex-col items-center flex-1">
                  ...
                  <span class="font-mono text-[20px] text-[var(--amber)] val-amber">{{ workbenchData?.aiResult.gradingLabel || 'G3' }}</span>
                </div>
                ...
                <div class="flex flex-col items-center flex-1">
                  ...
                  <span class="font-mono text-[20px] text-[var(--emerald)] val-emerald">{{ selectedGrade || 'G2' }}</span>
                </div>
              </div>
              ...
              <div class="grid grid-cols-3 gap-2 mt-auto">
                <NeuralButton variant="ghost" class="text-[11px] text-[var(--td)] border-[var(--td)]/30 hover:border-[var(--td)]">保存草稿</NeuralButton>
                <NeuralButton variant="danger" class="text-[11px]">二次意见</NeuralButton>
                <NeuralButton variant="success" @click="submitReview" :loading="submitting" class="text-[11px]">提交并记录</NeuralButton>
              </div>
            </div>
          </Panel>
        </div>
