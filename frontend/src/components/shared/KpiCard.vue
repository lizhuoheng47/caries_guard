<template>
  <div class="relative glass-panel rounded-md p-3 flex flex-col justify-between corner-cut-tr">
    ...
    <div class="flex items-center gap-1.5 mb-2">
      ...
      <span class="font-mono text-[11px] text-[var(--td)] uppercase tracking-[0.1em]">{{ label }}</span>
    </div>
    
    <!-- Value & Trend Row -->
    <div class="flex items-baseline gap-2">
      <span class="font-mono text-[24px] font-medium" :class="[`val-${color}`]">{{ value }}</span>
      <span v-if="trend" class="font-mono text-[10px]" :class="trend > 0 ? 'text-[var(--emerald)]' : 'text-[var(--amber)]'">
