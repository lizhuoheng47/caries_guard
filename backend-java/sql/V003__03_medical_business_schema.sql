...
-- 数据域：患者与病例基础、医学临床诊断链
-- 数据库版本建表规约脚本 V003
-- 说明：融合版本乐观锁、牙位证据溯源列、高敏防暴三列机制等。
-- ----------------------------

-- 13. 患者主档表 pat_patient (收敛紧急联系人，引入三列安全)
CREATE TABLE `pat_patient` (
  ...
  -- 出生日期(高敏视情况做单向加密或仅脱敏保护，这里维持普通 Date 结合列级脱敏策略也可，依照通用性使用三列思想扩充)
...
-- 14. 监护人信息表 pat_guardian (家庭联系人在此统一挂载)
...
-- 17. 病例主表 med_case (加入了 Version No)
...
-- 19. 病例诊断结论表 med_case_diagnosis (加入 Version No)
CREATE TABLE `med_case_diagnosis` (
  ...
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  /* 由于业务上容许一个病例多个分阶段 draft 共存，组合使用避免垃圾冗余数据导致异常 */
