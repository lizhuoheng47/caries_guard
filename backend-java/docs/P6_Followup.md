...
## 十四、阶段验收证据（2026-04-13）

### 14.1 已通过测试（in-memory / 设计先行）
...
1. 高风险报告后自动创建 `fup_plan` + 首个 `fup_task`。
2. 相同触发来源+触发引用幂等，不重复创建计划。
3. 低风险且无复查建议不触发 followup。
4. 任务标记 `OVERDUE` 时产生 `msg_notify_record(ALERT)` 留痕。
5. 单任务计划完成后计划状态自动关闭为 `DONE`。

### 14.2 已通过测试（boot 真实库 / Flyway）
...
1. `fup_plan` 持久化：`trigger_source_code=RISK_HIGH`，`trigger_ref_id=reportId`。
2. `fup_task` 持久化：`task_type_code=FOLLOW_CONTACT`，`task_status_code=TODO`。
3. `msg_notify_record` 持久化：`biz_module_code=FOLLOWUP`，`notify_type_code=REMINDER`。
4. `fup_record` 持久化后，关联任务进入 `DONE`，单任务计划自动关闭为 `DONE`。
5. 任务更新为 `OVERDUE` 后，会新增 `msg_notify_record(ALERT)`。
6. 病例状态最终为 `FOLLOWUP_REQUIRED`，且 `followup_required_flag=1`。

### 14.3 状态机证据链（主链路）

`ANALYZING -> REVIEW_PENDING -> REPORT_READY -> FOLLOWUP_REQUIRED`

对应 `change_reason_code`：
...
1. `med_case_status_log`：完整记录上述状态跳转。
2. `msg_notify_record`：记录随访任务创建提醒与逾期告警（ALERT）事件。
3. `rpt_export_log`：报告导出动作留痕仍持续生效，与 followup 链路不冲突。

### 14.5 未覆盖边界声明（当前阶段）

1. 通知发送当前采用 `msg_notify_record` 留痕，不接真实短信/邮件网关。
2. 逾期场景当前采用测试驱动状态流转验证，不依赖真实定时任务中间件调度。
3. analysis 侧消息链路仍处于 logging publisher 阶段，真实 MQ consumer 保留在后续 integration 阶段落地。
4. 医生人工复核后的随访触发逻辑暂由 Mock 规则模拟，尚未接入真实医生端 UI 的确认回调。
